...
%--- plot data0
subplot(4,1,1);
plot(0:len-1, real(data0), 'rx'); hold on;
if (option ~= 1)
plot(0:len-1, imag(data0), 'bo');;end
axis tight; grid on; title(Title0);
legend('实部','虚部')

%--- plot data1
subplot(4,1,2);
plot(0:len-1, real(data1), 'rx'); hold on;
if (option ~= 1)
plot(0:len-1, imag(data1), 'bo');  end
...
axis tight; grid on; title(['绝对误差:',strcat('(', Title0, ') - (', Title1, ')')]);
